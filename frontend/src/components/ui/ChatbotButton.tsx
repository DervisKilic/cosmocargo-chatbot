...
export default function ChatbotButton() {
  ...
  const sendMessage = (text: string) => {
    if (!text.trim()) return;
    setMessages((prev) => [...prev, { id: Math.random().toString(36).slice(2), role: "user", content: text.trim() }]);
    setInput("");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    sendMessage(text);
    setIsLoading(true);
    const history = messages.concat({ id: "tmp", role: "user" as const, content: text });
    const mapped = history.map((m) => ({ role: m.role === "bot" ? ("assistant" as const) : (m.role as "user"), content: m.content }));
    try {
      ...
      >
        <div className="flex items-center gap-3 bg-space-secondary text-white px-3 sm:px-4 py-3">
          ...
          <div className="flex-1 leading-tight">
            ...
            <div className="text-xs opacity-80">Ställ en fråga i textfältet nedan</div>
          </div>
          <button
        ...
        <div ref={scrollRef} role="log" aria-live="polite" className="sm:max-h-[60vh] max-h-[80vh] sm:min-h-[300px] min-h-[50vh] overflow-y-auto px-3 sm:px-4 py-3 space-y-3">
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[85%] sm:max-w-[80%] rounded-2xl px-3 py-2 text-sm shadow-sm ${
                  m.role === "user" ? "bg-space-accent-purple/90 text-white rounded-br-sm" : "bg-space-primary/80 border border-space-secondary/50 rounded-bl-sm"
                }`}
              >
                {m.content}
              </div>
        ...
        <form onSubmit={handleSubmit} className="border-t border-space-secondary/40 p-3 bg-space-primary/95">
